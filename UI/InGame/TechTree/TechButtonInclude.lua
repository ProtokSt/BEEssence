...
local g_HarmObjs : table = {
	"UNIT_XENO_SWARM_PROTOTYPE",
	"UNIT_XENO_CAVALRY_PROTOTYPE",
	"UNIT_ROCKTOPUS_PROTOTYPE",
	"UNIT_XENO_TITAN_PROTOTYPE",
	"UNIT_XENO_SWARM",
	"UNIT_XENO_CAVALRY",
	"UNIT_XENO_TITAN",
...
local g_PuriObjs : table = {
	"UNIT_BATTLESUIT_PROTOTYPE",
	"UNIT_AEGIS_PROTOTYPE",
	"UNIT_LEV_TANK_PROTOTYPE",
	"UNIT_LEV_DESTROYER_PROTOTYPE",
	"UNIT_BATTLESUIT",
	"UNIT_AEGIS",
	"UNIT_LEV_TANK",
	"UNIT_LEV_DESTROYER",
...
local g_SuprObjs : table = {
	"UNIT_CNDR_PROTOTYPE",
	"UNIT_CARVR_PROTOTYPE",
	"UNIT_SABR_PROTOTYPE",
	"UNIT_ANGEL_PROTOTYPE",
	"UNIT_CNDR",
	"UNIT_CARVR",
	"UNIT_SABR",
	"UNIT_ANGEL",
...
local g_SuprPuriObjs : table = {
	"UNIT_DRONE_CAGE_PROTOTYPE",
	"UNIT_AUTOSLED_PROTOTYPE",
	"UNIT_GOLEM_PROTOTYPE",
	"UNIT_DRONE_CAGE",
	"UNIT_AUTOSLED",
	"UNIT_GOLEM",
};

local g_PuriHarmObjs : table = {
	"UNIT_IMMORTAL_PROTOTYPE",
	"UNIT_ARCHITECT_PROTOTYPE",
	"UNIT_THRONE_PROTOTYPE",
	"UNIT_IMMORTAL",
	"UNIT_ARCHITECT",
	"UNIT_THRONE",
	"UNIT_ROCKTOPUS",
...
local g_HarmSuprObjs : table = {
	"UNIT_NANOHIVE_PROTOTYPE",
	"UNIT_GELIOPOD_PROTOTYPE",
	"UNIT_AQUILON_PROTOTYPE",
	"UNIT_NANOHIVE",
	"UNIT_GELIOPOD",
	"UNIT_AQUILON",
};
...
local framesize = 45
local m_textureFrameUnit : table		= { atlas="TECHWEB_ATLAS_FRAMES", size=framesize, index=0};
local m_textureFrameResource : table	= { atlas="TECHWEB_ATLAS_FRAMES", size=framesize, index=1};
local m_textureFrameImprovement : table = { atlas="TECHWEB_ATLAS_FRAMES", size=framesize, index=2};
local m_textureFrameBuilding : table	= { atlas="TECHWEB_ATLAS_FRAMES", size=framesize, index=3};
local m_textureFrameAbility : table		= { atlas="TECHWEB_ATLAS_FRAMES", size=framesize, index=4};
local m_textureFrameWonder : table		= { atlas="TECHWEB_ATLAS_FRAMES", size=framesize, index=5};
local m_textureFrameVictory : table		= { atlas="TECHWEB_ATLAS_FRAMES", size=framesize, index=6};
local m_textureFrameOrbital : table		= { atlas="TECHWEB_ATLAS_FRAMES", size=framesize, index=7};
local m_textureFramePrototype : table	= { atlas="TECHWEB_ATLAS_FRAMES", size=framesize, index=8};

local affinitysize = 27
local m_textureAffHarm : table			= { atlas="AFF_TECHWEB_ONDABTN_ATLAS", size=affinitysize, index=0};
local m_textureAffPuri : table			= { atlas="AFF_TECHWEB_ONDABTN_ATLAS", size=affinitysize, index=1};
local m_textureAffSupr : table 			= { atlas="AFF_TECHWEB_ONDABTN_ATLAS", size=affinitysize, index=2};
local m_textureAffPuriHarm : table		= { atlas="AFF_TECHWEB_ONDABTN_ATLAS", size=affinitysize, index=3};
local m_textureAffSuprPuri : table		= { atlas="AFF_TECHWEB_ONDABTN_ATLAS", size=affinitysize, index=4};
local m_textureAffHarmSupr : table		= { atlas="AFF_TECHWEB_ONDABTN_ATLAS", size=affinitysize, index=5};
...
function AddSmallButtonsToTechButton( thisTechButtonInstance, tech, maxSmallButtons, textureSize, startingButtonNum )
		...
		if (not ExcludeThisUnit(thisUnitInfo)) then
			...
			if validUnitBuilds[thisUnitInfo.Class] == thisUnitInfo.Type then
				...
				if thisButton then
					...
					thisButton["IconOndalay"] = thisTechButtonInstance["IconOndalay"..tostring(m_currentButtonNum)];	-- Attach ondalay control and give it common name.
	...
	for i,v in pairs(sortedYieldChanges) do
		...
		if thisButton ~= nil then
			...
			-- IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
			-- print(tostring(v[1]).." ".. tostring(GameInfo.Improvements[v[1]].PortraitIndex).. " ".. tostring(GameInfo.Improvements[v[1]].IconAtlas));
			IconHookup( GameInfo.Improvements[v[1]].PortraitIndex, textureSize, GameInfo.Improvements[v[1]].IconAtlas, thisButton );
			thisButton:SetHide( false );
	...
	if tech.AllowsDeepWaterCityMovement then
		...
		if thisButton then
			-- IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
			IconHookup( 0, textureSize, "FACILITY_ATLAS_PW1", thisButton );
			thisButton:SetHide( false );
	...
	if tech.UnitBaseHealModifier > 0 then
		...
		if thisButton then
			-- IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
			IconHookup( 3, textureSize, "UNIT_ACTION_ATLAS", thisButton );
			thisButton:SetHide( false );
...
--	RETURNS: true if so
-- ===========================================================================
function IsOrbital( unit )	--	PW
	local thisUnitType = GameInfo.Units[unit.Type];
	-- if (thisUnitType.Orbital == "NULL")	then -- or thisUnitType.Class == "UNITCLASS_ROCKTOPUS"
	if (thisUnitType.Orbital) and (thisUnitType.Class ~= "UNITCLASS_ROCKTOPUS")	then -- or thisUnitType.Class == "UNITCLASS_ROCKTOPUS"
		return true;
	end
	return false;	
end

function IsAffinity( objInfo : table )	--	PW
	
	--print("IsAffinity started!"); -- dbg
	
	for _,objType : string in ipairs(g_HarmObjs) do
		if (objType == objInfo.Type) then
			--print("IsAffinity return true!"); -- dbg
			return "Harm";
		end
	end		
	
	for _,objType : string in ipairs(g_PuriObjs) do
		if (objType == objInfo.Type) then
			--print("IsAffinity return true!"); -- dbg
			return "Puri";
		end
	...
	for _,objType : string in ipairs(g_SuprObjs) do
		if (objType == objInfo.Type) then
			--print("IsAffinity return true!"); -- dbg
			return "Supr";
		end
	...
	for _,objType : string in ipairs(g_SuprPuriObjs) do
		if (objType == objInfo.Type) then
			--print("IsAffinity return true!"); -- dbg
			return "SuprPuri";
		end
	...
	for _,objType : string in ipairs(g_PuriHarmObjs) do
		if (objType == objInfo.Type) then
			--print("IsAffinity return true!"); -- dbg
			return "PuriHarm";
		end
	...
	for _,objType : string in ipairs(g_HarmSuprObjs) do
		if (objType == objInfo.Type) then
			--print("IsAffinity return true!"); -- dbg
			return "HarmSupr";
		end
	...
	--print("IsAffinity return false!"); -- dbg
	return false;
...
function AdjustArtOnGrantedUnitButton( thisButton, thisUnitInfo, textureSize )
	...
	if thisButton then
		...
		if (GameInfo.Units[thisUnitInfo.Type].Prototype ~= true) then
			if (IsOrbital(thisUnitInfo)) then -- but not Rocktopus
				BackingFrameHookup(thisButton, m_textureFrameOrbital);
			else
				BackingFrameHookup(thisButton, m_textureFrameUnit);
			end
		else
			BackingFrameHookup(thisButton, m_textureFramePrototype);
		end

		local whatAff = IsAffinity(thisUnitInfo)
		if (whatAff ~= false) then
			if (whatAff == "Harm") then
				BackingAffinityHookup(thisButton, m_textureAffHarm);
			elseif (whatAff == "Puri") then
				BackingAffinityHookup(thisButton, m_textureAffPuri);
			elseif (whatAff == "Supr") then
				BackingAffinityHookup(thisButton, m_textureAffSupr);
			elseif (whatAff == "SuprPuri") then
				BackingAffinityHookup(thisButton, m_textureAffSuprPuri);
			elseif (whatAff == "PuriHarm") then
				BackingAffinityHookup(thisButton, m_textureAffPuriHarm);
			elseif (whatAff == "HarmSupr") then
				BackingAffinityHookup(thisButton, m_textureAffHarmSupr);
			end	
		end
	end
end
...
function AdjustArtOnGrantedBuildingButton( thisButton, thisBuildingInfo, textureSize )
	...
	if thisButton then
		...
		-- Backing aff icon
		local whatAff = IsAffinity(thisBuildingInfo)
		if (whatAff ~= false) then
			if (whatAff == "Harm") then
				BackingAffinityHookup(thisButton, m_textureAffHarm);
			elseif (whatAff == "Puri") then
...
function AdjustArtOnGrantedPlayerPerkButton (thisButton, thisPerkInfo, textureSize)
	if (thisButton ~= nil and thisPerkInfo ~= nil) then
		...
		-- Backing frame icon
		-- BackingFrameHookup(thisButton, m_textureFrameAbility);
		local perktype = thisPerkInfo.Type
		if (perktype=="PLAYERPERK_ALIEN_LEASH_MISSION") then 
			BackingFrameHookup(thisButton, m_textureFrameImprovement);
		else
			BackingFrameHookup(thisButton, m_textureFrameAbility);
		end
...
function BackingFrameHookup(buttonInstance : table, frameTextureTable : table)
	...
	-- local iconUnderlay = buttonInstance["IconOndalay"];
...
-- PW
function BackingAffinityHookup(buttonInstance : table, frameTextureTable : table)	
-- print("BackingAffinityHookup start!"); -- dbg
	if frameTextureTable == nil then
		return;
	end
	-- print("frameTextureTable not nil!"); -- dbg
	local IconOndalay = buttonInstance["IconOndalay"];
	if (IconOndalay ~= nil) then
		-- print("IconOndalay not nil!"); -- dbg
		local frameOffset : table, frameSheet : string = IconLookup( frameTextureTable.index, frameTextureTable.size, frameTextureTable.atlas );
		IconOndalay:SetTexture( frameSheet );
		IconOndalay:SetTextureOffset( frameOffset );
		IconOndalay:SetHide( false );
		IconOndalay["hasTexture"] = true;
	end
end
