...
function InputHandler( uiMsg, wParam, lParam )
	...
	if wParam == Keys.VK_CONTROL or hk_ctrl == true then 
		...
		elseif wParam == Keys.VK_F1 and uiMsg == KeyEvents.KeyDown and hk_ctrl == true then
			print("Ctrl hotkey down and F1 down ----") -- dbg  -- InterludePopup
			...
			Events.SerialEventGameMessagePopup( { Type = ButtonPopupTypes.BUTTONPOPUP_MODDER_1, Data1 = 0} );
			return true;
...
INTERL = ContextPtr:LoadNewContext("InterludePopup");--MGH:Load inside InterludePopup.lua + InterludePopup.xml as new UI Context
INTERL:SetHide(true);

--ContextPtr:LoadNewContext("InterludeControl");--MGH:(Do not exists InterludeControl.lua)
...
function InterludeControl(playerType)
	-- In the turn after first, 100 and 200 comparing playerType with the current active player
	-- In the turns 1,2,3,4
	if (Game:GetActivePlayer() == playerType) then--Player is the human game active player on screen
		if _dpo then print("MGH:InterludeControl Game:GetActivePlayer()="..tostring(Game:GetActivePlayer()).." playerType="..tostring(playerType).." Game.GetGameTurn="..tostring(Game.GetGameTurn()) ); end
		----
		if (Game.GetElapsedGameTurns()==1 or Game.GetElapsedGameTurns()==2 or Game.GetElapsedGameTurns()==3 or Game.GetElapsedGameTurns()==4) then --currentTurn
			--Data1 - 0/1
			--OnInterludePopup()
			--UIManager:PushModal(INTERL);
			UI.LookAt(Players[playerType]:GetCapitalCity():Plot(), 0);--MGH:Focus Map on Active Player Capital
			--MGH:open popup
			Events.AudioPlay2DSound("AS2D_INTERFACE_POLICY_CONFIRM");
...
-- PREGAME ACCESS
---------------------------------------------------------------------------------------
-- если такие проблемы с этой функцией, то проще записывать значение пока оно доступно в переменную сейва и из сейва считывать при необходимости.
