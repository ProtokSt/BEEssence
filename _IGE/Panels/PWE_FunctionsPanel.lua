...
function OnInitialize()
	print("PWE_FunctionsPanel.OnInitialize");
	SetUnitsData(data);
	...
	local tt = L("Additional functions of the script editor");
	-- LuaEvents.IGE_RegisterTab("FUNCTIONS", L("TXT_KEY_IGE_UNITS_PANEL"), 2, "paint",  tt, currentUnit)
	LuaEvents.IGE_RegisterTab("FUNCTIONS", L("Script Editor Function Panel"), 2, "paint",  tt, currentUnit)
	print("PWE_FunctionsPanel.OnInitialize - Done");
end
LuaEvents.IGE_Initialize.Add(OnInitialize)

-------------------------------------------------------------------------------------------------
function OnSelectedPanel(ID)
	isVisible = (ID == "FUNCTIONS");
end
...
function ClickHandler(unit)
	currentUnit = unit;
	OnUpdate();
	LuaEvents.IGE_SetTabData("FUNCTIONS", currentUnit);
end
