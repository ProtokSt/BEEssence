...
function OnSelectPlayer(ID, isShowing)
	...
	local safeMode = newPlayer:IsAlien() or newPlayer:IsMinorCiv() or IGE.safeMode
...
function OnUpdateOptions(options, suppressNotification)
	...
	if options then
		...
		if options.revealImprovements ~= nil then
			db.SetValue("RevealImprovements",				options.revealImprovements and "true" or "false");
		end
		if options.UIresize ~= nil then
			db.SetValue("UIresize",				options.UIresize and "true" or "false");
		end
	end
	...
	IGE.autoRevealImprovements = db.GetValue("RevealImprovements") ~= "false";
	IGE.UIresize = db.GetValue("UIresize") ~= "false";
