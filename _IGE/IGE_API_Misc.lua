...
function GetYieldString(item)
	...
	-- Straight yields : food, prod, science, culture, happiness, defense
	if item.yields then
...
function SaveFile(canCleanUpFile)
	...
	local text = L("TXT_KEY_IGE_SAVED_AS", fileName);
	Events.SerialEventGameMessagePopup{Type = ButtonPopupTypes.BUTTONPOPUP_TEXT, Data1 = 200, Option1 = false, Text = text};
	return fileName;
end