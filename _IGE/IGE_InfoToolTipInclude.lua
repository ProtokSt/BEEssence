...
-------------------------------------------------------------------------------------------------
function ReplaceHelpTextForUpgradedUnits(unit, activePlayer, bestUpgrade)
		local baseInfo = GameInfo.Units("Type = '"..unit.type.."'")();
		local perks = activePlayer:GetPerksForUnit(unit.ID);
		local freePerks = activePlayer:GetFreePerksForUnit(unit.ID);
		local upgradeInfo = GameInfo.UnitUpgrades[bestUpgrade];
		local moves = baseInfo.Moves;
		local range = baseInfo.Range;
		local rangeStrength = baseInfo.RangedCombat;
		local combatStrength = baseInfo.Combat;

		for i,v in ipairs(freePerks) do	table.insert(perks, v);	end

		for i,v in ipairs(perks) do
			local perkInfo = GameInfo.UnitPerks("ID = '"..v.."'")();
			if perkInfo.MovesChange ~= 0 then moves = moves + perkInfo.MovesChange;	end
			if perkInfo.RangeChange ~= 0 then range = range + perkInfo.RangeChange;	end
			if perkInfo.ExtraRangedCombatStrength ~= 0 then	rangeStrength = rangeStrength + perkInfo.ExtraRangedCombatStrength;	end
			if perkInfo.ExtraCombatStrength ~= 0 then combatStrength = combatStrength + perkInfo.ExtraCombatStrength; end
		end

		--Build help string for tooltip
		local help = "";
		help = help..L("TXT_KEY_PRODUCTION_COST", activePlayer:GetUnitProductionNeeded(unit.ID))
			.."[NEWLINE]"..L("TXT_KEY_PRODUCTION_MOVEMENT", moves);
		if (range ~= 0) then help = help.."[NEWLINE]"..L("TXT_KEY_PRODUCTION_RANGE", range); end
		if (rangeStrength ~= 0) then help = help.."[NEWLINE]"..L("TXT_KEY_PRODUCTION_RANGED_STRENGTH", rangeStrength); end
		if (combatStrength ~= 0) then help = help.."[NEWLINE]"..L("TXT_KEY_PRODUCTION_STRENGTH", combatStrength); end
		if baseInfo.Help then
			local subHelp = L(baseInfo.Help);
			if (strWrittenHelpText ~= nil and strWrittenHelpText ~= "") then
				help = help.."[NEWLINE]"..subHelp;
			end
		end
		if upgradeInfo.Help then
			local subHelp = L(upgradeInfo.Help);
			if (strWrittenHelpText ~= nil and strWrittenHelpText ~= "") then
				help = help.."[NEWLINE]"..subHelp;
			end
		end
		if (baseInfo.Requirements) then
			help = help.."[NEWLINE]"..L(baseInfo.Requirements);
		end

		unit.help = help;			
		AppendIDAndTypeToHelp(unit);
end