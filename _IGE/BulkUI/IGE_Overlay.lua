...
local function UpdateToolTipCore(plot)
	...
	if str ~= "" then
		tipControls.IGETIPText:SetText(str);
		tipControls.IGEGrid:DoAutoSize();
		Controls.IGEBox:SetToolTipType("HexDetails");
	else
		Controls.IGEBox:SetToolTipType();
	end
end
...
local function UpdateYieldOverlay(plot, index, x, y)
	...
	AppendYieldIcon(4, plot:CalculateYield(4, true), plotInstance.Stack, plotInstance.images);
	AppendYieldIcon(5, plot:CalculateYield(5, true), plotInstance.Stack, plotInstance.images);
    ...
    plotInstance.Anchor:SetWorldPosition( LocalWorldPos );
end
...
local function UpdateResourceOverlay(plot, index, x, y)
	...
	local x, y, z = GridToWorldClamped( x, y ); -- Keep above water.
...
local function UpdateOverlay()
	...
	Controls.OverlayResourceContainer:SetHide(not IGE.showResources);
