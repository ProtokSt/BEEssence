...
function OnInitialize()
	...
	--Resize Main panel
	Controls.MainGrid:SetSizeVal(mainGridX, mainGridY + 33);
	--Resize MainBox OffsetY 33
	Controls.MainBox:SetSizeVal(mainGridX, mainGridY);
	--Resize PanelsGrid OffsetY 0
	Controls.PanelsGrid:SetSizeVal(mainGridX, mainGridY);
	--Resize PanelsContainer OffsetY 46
	Controls.PanelsContainer:SetSizeVal((mainGridX - 10), (mainGridY - 46));

	-- Offsetting main panel
	Controls.MainGrid:SetOffsetY(mainGridOffsetY);
	Controls.MainGrid:SetOffsetX(mainGridOffsetX);

	local sizebX, sizebY = UIManager:GetScreenSizeVal();
	Controls.Background:SetSizeVal(sizebX, sizebY);
	Controls.Background:SetOffsetY(0);
	Controls.Background:SetOffsetX(0);
...
-------------------------------------------------------------------------------------------------
local function ClosePopups(close)
	if close then		
		LookUpControl("InGame/WorldView/InfoCorner/TechPanel"):SetHide(false);
		else
	...
	LookUpControl("InGame/VirtuesPopup"):SetHide(true);
...
local function OnInitializationError(err)
	...
	-- Show popup to user
	local str = L("TXT_KEY_IGE_LOADING_ERROR").."[NEWLINE][ICON_BULLET] "..err;
...
function InputHandler(uiMsg, wParam, lParam)
	if IsVisible() then
		...
		local test1 = Controls.Background:HasMouseOver();
...
function OnUpdatedOptions(IGE)
	...
	Controls.RevealImprovements:SetCheck(IGE.autoRevealImprovements);
	Controls.UIresize:SetCheck(IGE.UIresize);
end
...
function OnOptionControlChanged()
	...
	options.revealImprovements = Controls.RevealImprovements:IsChecked();
	options.UIresize = Controls.UIresize:IsChecked();
...
Controls.RevealImprovements:RegisterCheckHandler(OnOptionControlChanged);
Controls.UIresize:RegisterCheckHandler(OnOptionControlChanged);
