...
-------------------------------------------------------------------------------------------------
local function OnRemoveMiamsa(revealing, disableFoW)
	for i = 0, Map.GetNumPlots()-1, 1 do
		local plot = Map.GetPlotByIndex(i);
		if plot:HasMiasma() then plot:SetMiasma(false) end
	end
end
LuaEvents.PWE_RemoveMiamsa.Add(OnRemoveMiamsa);

-------------------------------------------------------------------------------------------------
local function OnRemovePods(revealing, disableFoW)
	for i = 0, Map.GetNumPlots()-1, 1 do
		local plot = Map.GetPlotByIndex(i);
		if plot:IsGoody() == true then plot:RemoveGoody(); end
	end
	Game.UpdateFOW(true);
	UI.RequestMinimapBroadcast();
end
...
-------------------------------------------------------------------------------------------------
local function OnRemoveAliens(revealing, disableFoW)
	for i = 0, Map.GetNumPlots()-1, 1 do
		local plot = Map.GetPlotByIndex(i);
		local unit = plot:GetUnit()
		if unit ~= nil then
			if  Players[unit:GetOwner()] == Players[GameDefines.ALIEN_PLAYER] then unit:Kill(true, -1); end
		end
		-- if there second unit on tile
		unit = plot:GetUnit()
		if unit ~= nil then
			if  Players[unit:GetOwner()] == Players[GameDefines.ALIEN_PLAYER] then unit:Kill(true, -1); end
		end
...
LuaEvents.PWE_RemoveAliens.Add(OnRemoveAliens);

-------------------------------------------------------------------------------------------------
local function OnSetWild0Click(revealing, disableFoW)
	for i = 0, Map.GetNumPlots()-1, 1 do
		local plot = Map.GetPlotByIndex(i);
		plot:SetWildness(0);
	end
