...
--------------------------------------------------------------
local db = Modding.OpenUserData("IGEditorBE", 4);
local UIresize = db.GetValue("UIresize") ~= "false";
...
local sizeOX, sizeOY = UIManager:GetScreenSizeVal();
mainGridOffsetX = (sizeOX - mainGridX) - 10;
mainGridOffsetY = (sizeOY - mainGridY) - 40;
--Inner panels
g_panelSizeX = mainGridX - 20;
g_panelSizeY = mainGridY - 46;
if g_panelSizeY > 300 then
	g_listsSizeY = 274;
else
	g_listsSizeY =  g_panelSizeY - 26;
end
--Special
affinityPanelX = (g_panelSizeX - 5) / 3;
citypanelsplitX = (g_panelSizeX - 200) / 2;
...
for i, v in ipairs(packageIDs) do
	...
	Active = bActive;
	if ID == "54D2B257-C591-4045-8F17-A69F033166C7" then
		if Active then		
			IGE_HasRisingTide = true;
			--print("Rising Tide DLC: true");
		else
			IGE_HasRisingTide = false;
			--print("Rising Tide DLC: false");
		end
	end
end
-------------------------------------------------------------------------------------------------
