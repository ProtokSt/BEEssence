...
--	EDITED:   Many changes to health childhood diseases and to make more options.
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
--  Adding DBG logs for better understanding of resource placing changes problems
--	AssignStartingPlots:Create
--		ignoreWildness
--	AssignStartingPlots:EvaluateCandidatePlot
--		ignoreWildness
--	AssignStartingPlots:CreateResources, fixing some bugs, enhance for subOption 7
--  AssignStartingPlots:Quantities, enhance for subOption 7
--  AssignStartingPlots:SetResources, dbg log and bug fix
--	AssignStartingPlots:BalanceCivStarts()
--	AssignStartingPlots:BasicFood(food)
--	AssignStartingPlots:BasicEnergyProduction(resource, isProduction)
--	AssignStartingPlots:AffinityThirdRing(resource)
--  AssignStartingPlots:ChooseLocations(args) - added args 
--  AssignStartingPlots:FindStart(region_number)  -  self.chooseStartCondition check
--  AssignStartingPlots:FindStartWithoutRegardToAreaID(region_number)  -  self.chooseStartCondition check
--  AssignStartingPlots:FindAllStart(region_number)  -  self.chooseStartCondition check
...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		BalanceCivStarts = AssignStartingPlots.BalanceCivStarts, -- Called from PlaceResourcesAndCityStates
		...
		BasicEnergyProduction = AssignStartingPlots.BasicEnergyProduction, -- Called from BalanceCivStarts
			AffinityThirdRing = AssignStartingPlots.AffinityThirdRing, -- Called from BalanceCivStarts. By Protok
			CompetitionStart = AssignStartingPlots.CompetitionStart, -- Called from BalanceCivStarts. By Protok
		...
		method = 2,						-- Method of regional division, default is 2 - Continental
		...
		-- new args for BalanceAndAssign
		chooseStartCondition = 3,		-- 3 is condition that prevent spawning less then 3 tiles away from map border. 
		startLocs = false,				-- a list of indexes or false

		
		-- Team info variables (not used in the core process, but necessary to many Multiplayer map scripts)
		bTeamGame,						-- #Players > #Teams
		iNumTeamsOfCivs,
		teams_with_major_civs,
		number_civs_per_team,
		iNumTeams = -1,					-- Number of Teams at game start. AC
		teamWestID = -1,				-- For teamed region division
		teamEastID = -1, 				-- For teamed region division
		iNumCivsInWest = -1,			-- For teamed region division
		iNumCivsInEast = -1,			-- For teamed region division
		...
		-- a list of resources with this specific
		basic_food = {};
		basic_production = {};
		basic_energy = {};
		basic_culture = {},
...
function AssignStartingPlots:MeasureStartPlacementFertilityOfPlot(x, y)
	...
	if plot:HasMiasma() and (not self.ignoreMiasmaToFertility) then
...
function CalculateFertilityMap()
	...
	local closeFertilities = { 0, 1, 1, 1, 2, 2, 2, 3 };
	local farFertilities = { 0, 1, 1, 2, 2, 3, 3, 4 };
	...
	for _,plot in Plots() do
		local distFromCentroid = MapGenerator.GetPlotHexDistanceFromRegionCentroid(plot);

		local randomFertility = 1;
	...
	local ringMedianFertility = 3;
	local maxIdealFertility = 4;
	local varianceAbove = maxIdealFertility - ringMedianFertility;
	local varianceBelow = -1;

	for _,plot in ipairs(centroidPlots) do
		for ringRadius=1,3 do
			local ringPlots = {};
			AssignStartingPlots.DoForEachRingPlot(plot, ringRadius, function(neighborPlot) 
				table.insert(ringPlots, neighborPlot);
			end);

			local variances = {};
			for variance =1,#ringPlots / 2 do
				local variance = Game.RandRange(varianceBelow, varianceAbove, "Calculating plot fertility variance");
...
function AssignStartingPlots:GenerateRegions(args)
	...
	--				   Default if map gives no args.method
	...
	-- 5. Whole Map:   Civs start all over the map randomly with set of fertility check.
	-- 				   Usual for BERT maps.
	-- 6. Teams W/E:   Works only if 2 teams of players present. Teams spawn on west or east sides of map.
	--
	local args = args or {};
	local iW, iH = Map.GetGridSize();
	...
	self.ignoreMiasmaToFertility = args.ignoreMiasmaToFertility or false; -- Will ignore Miasma in fertility calculation.
	self.excludeTiles = args.excludeTiles or false; -- list of indexes to exclude from res gen
	print("Method: "..tostring(self.method)..", Resources: "..tostring(self.resource_setting));
	...
	self.iNumTeams = table.maxn(self.teams_with_major_civs);
	print("-"); print("Teams:", self.iNumTeams);
	...
	elseif  self.method == 5 then -- Whole Map
		...
		self:DivideIntoRegions(self.iNumCivs, fert_table, rect_table)
		-- The regions have been defined.
	elseif  (self.method == 6 and self.iNumTeams == 2) then -- Teams W/E
		-- If two teams are present, use team-oriented handling of start points, one team west, one east.
		print("-"); print("Number of Teams present is two! Using team start placement by West and East."); print("-");
		...
		self.teamWestID = shuffled_team_list[1];
		self.teamEastID = shuffled_team_list[2];
		self.iNumCivsInWest = self.number_civs_per_team[self.teamWestID];
		self.iNumCivsInEast = self.number_civs_per_team[self.teamEastID];
		if _dpo then print("self.iNumCivsInWest = "..tostring(self.iNumCivsInWest)..", self.iNumCivsInEast = "..tostring(self.iNumCivsInEast)) end
		...
		local rect_table = {self.inhabited_WestX, self.inhabited_SouthY, self.inhabited_Width,
							self.inhabited_Height, -1, fertCount, plotCount}; -- AreaID -1 means ignore area IDs.
		-- Divide the rectangle.
		self:DivideIntoRegions(self.iNumCivsInWest, fert_table, rect_table)
		...
		local rect_table = {self.inhabited_WestX, self.inhabited_SouthY, self.inhabited_Width,
							self.inhabited_Height, -1, fertCount, plotCount}; -- AreaID -1 means ignore area IDs.
		-- Divide the rectangle.
		self:DivideIntoRegions(self.iNumCivsInEast, fert_table, rect_table)
		-- The regions have been defined.

		-- If number of teams is any number other than two, use Continental division.
	else -- Continental.
		if self.method ~= 2 then
			self.method = 2
			if _dpo then print("Method of region division was switched to self.method = 2") end
		end
...
function AssignStartingPlots:EvaluateCandidatePlot(plotIndex, region_type)
	...
	-- ignore wildness for fertility
	if self.ignoreWildness then wild_area_score = 0 end
...
function AssignStartingPlots:FindStart(region_number)
	print("AssignStartingPlots:FindStart(region_number = "..tostring(region_number)..")");
	...
	for region_y = 0, iHeight - 1 do -- When handling global plot indices, process Y first.
		for region_x = 0, iWidth - 1 do
			...
			local plot = Map.GetPlot(x, y);
			---- Main check of 3 tiles away form map borders
			-- work a startLocCondition
			local tilepass = true
			if self.chooseStartCondition ~= 3 then
				tilepass = true
			else
				tilepass = false
				if (y > 2 and y < (iH-4)) then
					if Map:IsWrapX() then
						tilepass = true
					elseif (x > 2 and x < (iW-4)) then
						tilepass = true
					end
				end
			end
	...
	if iNumFallbacks > 0 then
		...
		if self.chooseStartCondition ~= 3 then
			...
			-- WORK Y
			if (iSouthY > 2 and iSouthY < (iH-4)) then

			elseif iSouthY < 2 then 
				iSouthY = iSouthY + 3
				forcePlot = Map.GetPlot(iWestX, iSouthY);
			elseif iSouthY > (iH-4) then 
				iSouthY = iSouthY - 3
				forcePlot = Map.GetPlot(iWestX, iSouthY);
			end
			...
			elseif (iWestX > 2 and iWestX < (iW-4)) then
			
			elseif iWestX < 2 then 
				iWestX = iWestX + 3
				forcePlot = Map.GetPlot(iWestX, iSouthY);
			elseif iWestX > (iW-4) then 
				iWestX = iWestX - 3
				forcePlot = Map.GetPlot(iWestX, iSouthY);
			end
		end	
...
function AssignStartingPlots:FindStartWithoutRegardToAreaID(region_number)
	print("AssignStartingPlots:FindStartWithoutRegardToAreaID(region_number = "..tostring(region_number)..")");
	...
	-- Debug printout.
	if _dpo then print("-"); end
	if _dpo then print("--- Number of Candidate Plots in each landmass in Region #", region_number, " - Region Type:", region_type, " ---"); end
	if _dpo then print("-"); end
	for loop, iAreaID in ipairs(best_areas) do
		...
		if _dpo then print("* Area ID#", iAreaID, "has fertility rating of", fert_rating, "and candidate plot count of", plotCount); print("-"); end
	end
	if _dpo then print("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -"); end
	--
...
function AssignStartingPlots:FindAllStart(region_number)
	print("AssignStartingPlots:FindAllStart(region_number = "..tostring(region_number)..")");
	-- This function attempts to choose a start position for a single region.
	-- This function returns two boolean flags, indicating the success level of the operation.
	local bSuccessFlag = false; -- Returns true when a start is placed, false when process fails.
	local bForcedPlacementFlag = false; -- Returns true if this region had no eligible starts and one was forced to occur.
	local _dpo = true;
	-- local _dpo = false;
	if _dpo then print("All Start"); end
	...
	-- Debug printout.
	if _dpo then print("-");	end
	if _dpo then print("--- Number of Candidate Plots in Region #", region_number, " - Region Type:", region_type, " ---");	end
	if _dpo then print("-");	end
	if _dpo then print("Candidates in Center Bias area: ", iNumCenter);	end
	if _dpo then print("Which are next to river: ", iNumCenterRiver);	end
	if _dpo then print("Which are in sea: ", iNumCenterCoast);	end
	if _dpo then print("Which are next to lake or sea: ", iNumCenterCoastal);	end
	if _dpo then print("Which are inland and dry: ", iNumCenterInlandDry);	end
	if _dpo then print("-");	end
	if _dpo then print("Candidates in Middle Donut area: ", iNumMiddle);	end
	if _dpo then print("Which are next to river: ", iNumMiddleRiver);	end
	if _dpo then print("Which are in sea: ", iNumMiddleCoast);	end
	if _dpo then print("Which are next to lake or sea: ", iNumMiddleCoastal);	end
	if _dpo then print("Which are inland and dry: ", iNumMiddleInlandDry);	end
	if _dpo then print("-");	end
	if _dpo then print("Candidate Plots in Outer area: ", iNumOuter);	end
	if _dpo then print("-");	end
	if _dpo then print("Candidate Plots in Outer Coast area: ", iNumOuterCoast);	end
	if _dpo then print("-");	end
	if _dpo then print("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -");	end
	--
	...
	-- STARTING LAND STEP 1 - MAIN LISTS OF TILES
	if landPlots > 0 then
		local candidate_lists = {};
		...
		--
		for loop, plot_list in ipairs(candidate_lists) do -- Up to six plot lists, processed by priority.
			if _dpo then print("Region #"..tostring(region_number).." STARTING LAND STEP 1. LOOP = "..tostring(loop).." table size = "..tostring(table.maxn(plot_list))); end
			local election_returns = self:IterateThroughCandidatePlotList(plot_list, region_type)
			-- If any candidates are eligible, choose one.
			local found_eligible = election_returns[1];
			if found_eligible and not foundLand then
				...
				self:PlaceImpactAndRipples(x, y)
				foundLand = true;
				if _dpo then print("Region #"..tostring(region_number).." foundLand "..tostring(x)..", "..tostring(y)); end
			end
			-- If none eligible, check for fallback plot.
			local found_fallback = election_returns[4];
			if found_fallback then
	...
	-- STARTING LAND STEP 2 - OUTER LIST OF TILES
	-- Reaching this point means no eligible sites in center bias or middle donut subregions!
	-- Process candidates from Outer subregion, if any.
	if iNumOuter > 0 and not foundLand then
		...
		if found_eligible and not foundLand then -- Iterate through eligible plots and choose the one closest to the center of the region.
			...
			if _dpo then print("Region #"..tostring(region_number).." STARTING LAND STEP 2. table size = "..tostring(table.maxn(outer_eligible_list))); end
			for loop, plotIndex in ipairs(outer_eligible_list) do
				local x = (plotIndex - 1) % iW;
				local y = (plotIndex - x - 1) / iW;
				local adjusted_x = x;
	...
	-- STARTING LAND STEP 3 - FALLBACK LIST OF TILES
	-- Reaching here means no plot in the entire region met the minimum standards for selection.
	-- The fallback plot contains the best-scored plots from each test area in this region.
	-- We will compare all the fallback plots and choose the best to be the start plot.
	local iNumFallbacks = table.maxn(fallback_plots);
	if iNumFallbacks > 0 and not foundLand then
		...
		local best_fallback_x;
		local best_fallback_y;
		if _dpo then print("Region #"..tostring(region_number).." STARTING LAND STEP 3. table size = "..tostring(iNumFallbacks)); end
		for loop, plotData in ipairs(fallback_plots) do
			local score = plotData[3];
			if score > best_fallback_score then
				best_fallback_score = score;
		...
		self:PlaceImpactAndRipples(best_fallback_x, best_fallback_y)
		bSuccessFlag = true;
		if _dpo then print("Region #"..tostring(region_number).." foundLand "..tostring(best_fallback_x)..", "..tostring(best_fallback_y)); end
	elseif not foundLand then
	-- STARTING LAND STEP 4 - FORCE A TILE
		-- This region cannot have a start and something has gone way wrong.
		-- We'll force a one tile grass island in the SW corner of the region and put the start there.
		local forcePlot = Map.GetPlot(iWestX, iSouthY);
		if _dpo then print("Region #"..tostring(region_number).." STARTING LAND STEP 4. force to "..tostring(iWestX)..", "..tostring(iSouthY)); end
		bSuccessFlag = true;
		bForcedPlacementFlag = true;
		-- work a startLocCondition
		if self.chooseStartCondition ~= 3 then
		...
		self.startingPlots[region_number] = {iWestX, iSouthY, 0};
		self:PlaceImpactAndRipples(iWestX, iSouthY)
		if _dpo then print("Region #"..tostring(region_number).." force land "..tostring(iWestX)..", "..tostring(iSouthY)); end
	end
	

	-- Process lists of water candidate plots.
	-- STARTING WATER STEP 1 - MAIN LISTS OF TILES
	if iNumCenterCoast + iNumMiddleCoast > 0 then
		...
		--
		for loop, plot_list in ipairs(candidate_lists) do -- Up to six plot lists, processed by priority.
			if _dpo then print("Region #"..tostring(region_number).." STARTING WATER STEP 1. LOOP = "..tostring(loop).." table size = "..tostring(table.maxn(plot_list))); end
			local election_returns = self:IterateThroughCandidatePlotList(plot_list, region_type)
			-- If any candidates are eligible, choose one.
			local found_eligible = election_returns[1];
			if found_eligible then
				...
				self:PlaceImpactAndRipples(x, y)
				if _dpo then print("Region #"..tostring(region_number).." foundWater "..tostring(x)..", "..tostring(y)); end
				return true, false
			end
			-- If none eligible, check for fallback plot.
			local found_fallback = election_returns[4];
	...
	-- STARTING WATER STEP 2 - OUTER LIST OF TILES
	-- Process candidates from Water Outer subregion, if any.
	if iNumOuterCoast > 0 then
		...
		if found_eligible then -- Iterate through eligible plots and choose the one closest to the center of the region.
			...
			if _dpo then print("Region #"..tostring(region_number).." STARTING WATER STEP 2. table size = "..tostring(table.maxn(outer_eligible_list))); end
			for loop, plotIndex in ipairs(outer_eligible_list) do
				local x = (plotIndex - 1) % iW;
				local y = (plotIndex - x - 1) / iW;
				local adjusted_x = x;
	...
	-- Reaching here means no plot in the entire region met the minimum standards for selection.
	-- STARTING WATER STEP 3 - FALLBACK  LIST OF TILES
	-- The fallback plot contains the best-scored plots from each test area in this region.
	-- We will compare all the fallback plots and choose the best to be the start plot.
	...
	if iNumFallbacks > 0 then
		...
		local best_fallback_x;
		local best_fallback_y;
		if _dpo then print("Region #"..tostring(region_number).." STARTING WATER STEP 3. table size = "..tostring(iNumFallbacks)); end
		...
		self:PlaceImpactAndRipples(best_fallback_x, best_fallback_y)
		if _dpo then print("Region #"..tostring(region_number).." foundWater "..tostring(best_fallback_x)..", "..tostring(best_fallback_y)); end
	else
	-- STARTING LAND WATER 4 - FORCE A TILE
		-- This region cannot have a start and something has gone way wrong.
		-- We'll force a one tile grass island in the SW corner of the region and put the start there.
		local forcePlot = Map.GetPlot(iWestX, iSouthY);
		-- work a startLocCondition
		...
		self:PlaceImpactAndRipples(iWestX, iSouthY)
		if _dpo then print("Region #"..tostring(region_number).." force water "..tostring(iWestX)..", "..tostring(iSouthY)); end
	end
...
function AssignStartingPlots:ChooseLocations(args)
	print("Map Generation - Choosing Start Locations for Civilizations");	-- dbg print out
	local _dpo = true;
	-- local _dpo = false;
	local args = args or {};
	self.chooseStartCondition = args.chooseStartCondition or self.chooseStartCondition; -- 3 - is default for 3 tiles away from border of map.
	self.startLocs = args.startLocs or self.startLocs; -- a list of indexes
	-- 
	local iW, iH = Map.GetGridSize();
	...
	self.ignoreWildness = args.ignoreWildness or false; -- if true, will ignore Wildness of tile area in Evaluation of fertility
	...
	for assignIndex = 1, iNumRegions do
		...
		-- do fixed starting locations, first part
		-- if self.startLocs and self.startLocs[("player"..assignIndex)] then
		if self.startLocs and self.startLocs[assignIndex] then
			-- local sp_index = self.startLocs[("player"..assignIndex)]
			local sp_index = self.startLocs[assignIndex]
			local start_plot = Map.GetPlotByIndex(sp_index)
			local sp_X = start_plot:GetX()
			local sp_Y = start_plot:GetY()
			local region_type = self.regionTypes[currentRegionNumber];
			local score, meets_minimums = self:EvaluateCandidatePlot(sp_index, region_type)
			self.startingPlots[currentRegionNumber] = {sp_X, sp_Y, score};
			self.startingWaterPlots[currentRegionNumber] = {sp_X, sp_Y, score};
			self:PlaceImpactAndRipples(sp_X, sp_Y)
			if _dpo then print("Region #"..tostring(currentRegionNumber).." Fixed Start Location "..tostring(sp_X)..", "..tostring(sp_Y)); end
		...
		-- if (not self.startLocs) or (not self.startLocs[("player"..assignIndex)]) then
		if (not self.startLocs) or (not self.startLocs[assignIndex]) then
			-- [[ Printout for debug only.
			if _dpo then print("- - -"); end
			if _dpo then print("Start Plot for Region #", currentRegionNumber, " was successful: ", bSuccessFlag); end
			if _dpo then print("Start Plot for Region #", currentRegionNumber, " was forced: ", bForcedPlacementFlag); end
			-- ]] --		
		end
	end
	--
	...
	if _dpo then print("-"); end
	if _dpo then print("--- Table of results, Start Finder self.startingPlots ---"); end
	for loop, startData in ipairs(self.startingPlots) do
		if _dpo then print("-"); end
		if _dpo then print("Region#", loop, " has start plot at: ", startData[1], startData[2], "with Fertility Rating of ", startData[3]); end
	end
	if _dpo then print("-"); end
	if _dpo then print("--- Table of results, Start Finder ---"); end
	if _dpo then print("-"); end
	--
	
	-- Printout of start plots. Debug use only.
	if _dpo then print("-"); end
	if _dpo then print("--- Table of results, Start Finder self.startingWaterPlots ---"); end
	...
	-- if _dpo then print("--- Impact and Ripple ---"); end
	-- if _dpo then PrintContentsOfTable(self.distanceData) end
	-- if _dpo then print("-");  end
	-- ]] --
end
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
function AssignStartingPlots:CreateResources(fertilityMap)
	print("Creating Resources - Weighted Density method (AssignStartingPlots.Lua), table.getn(fertilityMap) "..tostring(table.getn(fertilityMap)));
	...
	-- �� ������ ����������� ������, � ���� ������. ����� ������� ���������� � ������� �� 100.
	-- ������������ �� ������� ���-�� �������� �� �����, � ��������� ��������� ����������� ��� ������ ������.
	local landDensity = 20;
	local seaDensity = 30;
	-- local seaDensity = 20; -- because of REEF availability. OBSOLETE. Done with universal check below

	local gridWidth, gridHeight = Map.GetGridSize();
	local baseRunningWeight = ((gridWidth * gridHeight) * (landDensity / 4)) / 100;
	local resourceTrackingTable = {};
	
	if _dpo then print("landDensity = "..tostring(landDensity)..", gridWidth = "..tostring(gridWidth)..", gridHeight = "..tostring(gridHeight)); end
	if _dpo then print("baseRunningWeight = ((gridWidth * gridHeight) * (landDensity / 4)) / 100 = "..tostring(baseRunningWeight)); end
	...
	for row in GameInfo.Resources("ResourceClassType = 'RESOURCECLASS_BASIC' OR ResourceClassType = 'RESOURCECLASS_STRATEGIC'") do
		...
		if _dpo then print(row.ResourceClassType.." "..row.Type.." RunningWeight = baseRunningWeight = "..baseRunningWeight..", NegativeWeight = "..row.NegativeWeight); end
	end

	local plotResourceWeights = {};
	
	-- dbg
	local iplots = 0;
	local idensitypass = 0;
	local enor = 0;
	local enorS = 0;
	local enorB = 0;
	
	-- dbg, shuffling plots, for correcting underweighted (RunningWeight) rare resources in the future
	-- and equalize weights now
	-- local shuffledPlots = {};
	-- for iPlotID = 1, Map.GetNumPlots() do
		-- local pPlot = Map.GetPlotByIndex(iPlotID-1);
		-- shuffledPlots[iPlotID] = pPlot;
	-- end;
	-- shuffledPlots = GetShuffledCopyOfTable(shuffledPlots);	
	
	-- for i, plot in Plots() do
	for i, plot in Plots(Shuffle) do	-- built-in shuffle
	-- for i=1, #shuffledPlots do
		-- local plot = shuffledPlots[i];
		iplots = iplots +1
		-- here we should check the excluded tiles of map
		local randChance = Map.Rand(100, "Resource random chance");
		if self.excludeTiles then
			for j = 1, table.getn(self.excludeTiles), 1 do
				local check_plot_index = plot:GetPlotIndex()
				local exclude_plot = Map.GetPlotByIndex(self.excludeTiles[j])
				-- if _dpo then print("check_plot_index "..tostring(check_plot_index)..", "..plot:GetX()..", "..plot:GetY().." and self.excludeTiles["..j.."] "..tostring(self.excludeTiles[j])); end
				if check_plot_index == self.excludeTiles[j] then
					if _dpo then print("check_plot_index "..tostring(check_plot_index)..", "..plot:GetX()..", "..plot:GetY().." and self.excludeTiles["..j.."] "..tostring(self.excludeTiles[j])); end
					randChance = 1000
					break
				end
		...
		-- local randChance = Map.Rand(100, "Resource random chance");
		local density = landDensity;
		
		-- checking water density 30 for unreefed vanilla and turn back 20 for reefed ACDLC
		if (plot:IsWater()) then
			density = seaDensity;
			for row in GameInfo.Resource_FeatureBooleans("ResourceType = 'RESOURCE_TITANIUM'") do
				if _dpo then print("Resource_FeatureBooleans has titanium then reef allowed then seaDensity must be 20"); end
				density = landDensity
			end
		end
		
		if (randChance <= density) then
			idensitypass = idensitypass +1
			plotResourceWeights = {};
			local resourceID = -1;
			local totalWeight = 0;

			-- Compile weighted table of valid resources for this plot
			for id, resourceInfo in pairs(resourceTrackingTable) do
			...
			if #plotResourceWeights > 0 then
				local weightRoll = Map.Rand(totalWeight, "Resource weight roll");
				for j, pair in ipairs(plotResourceWeights) do
			...
			if (resourceID >= 0) then				
				...
				if (resourceInfo.ResourceClassType == "RESOURCECLASS_STRATEGIC") then					
					...
					for resourceQuantityInfo in GameInfo.Resource_QuantityTypes("ResourceType = \"" .. resourceInfo.Type .. "\"") do
						...
						-- real random option
						elseif (resourceQuantityInfo.Setting == 7) then
							inputSetting = resourceQuantityInfo.Setting;
						end
						...
						if (resourceQuantityInfo.Max ~= nil and resourceQuantityInfo.Min ~= nil) then
							if (inputSetting == self.resource_setting or (self.resource_setting == 4 and inputSetting ~= 7) or (self.resource_setting == 5 and inputSetting ~= 7) or (self.resource_setting == 8 and inputSetting ~= 7)) then
								...
								if _dpo then print("Min/Max: "..min.."/"..max.." "..resourceInfo.Type.." Quantity: "..quantityNumber); end
					...
					if (#quantity > 0 and amountTotal > 0) then
						local resourceAmount = Game.Rand(amountTotal * randomFactor, "Choosing quantity") + 1;
						--Go through a weighted list. The higher weight replaces the lower weight
						for j = 1, amountTotal, 1 do
							
							-- original for resorces map option 1,2,3
							if amountTotal <= 2 then
								if _dpo then print(resourceAmount.." > "..tostring(randomFactor * (j + 1))); end
								-- if(resourceAmount > (randomFactor * j) / amountTotal + randomFactor / amountTotal or j == 1) then
								-- same but understandable
								if(resourceAmount > (randomFactor * (j + 1)) / amountTotal or j == 1) then
									amount = quantity[j];
									if _dpo then print("amount = "..amount); end
								end
								
							-- for correct weight of quantity when amountTotal = 6, for option 4, 5
							elseif amountTotal == 6	then  
								if j == 1 and resourceAmount <= 5 then
									amount = quantity[j];
									if _dpo then print(resourceAmount..", amount = "..amount); end
								elseif j == 2 and resourceAmount > 5 and resourceAmount <= 20 then
									amount = quantity[j];
									if _dpo then print(resourceAmount..", amount = "..amount); end
								elseif j == 3 and resourceAmount > 20 and resourceAmount <= 30 then
									amount = quantity[j];
									if _dpo then print(resourceAmount..", amount = "..amount); end
								elseif j == 4 and resourceAmount > 30 and resourceAmount <= 40 then
									amount = quantity[j];
									if _dpo then print(resourceAmount..", amount = "..amount); end
								elseif j == 5 and resourceAmount > 40 and resourceAmount <= 55 then
									amount = quantity[j];
									if _dpo then print(resourceAmount..", amount = "..amount); end
								elseif j == 6 and resourceAmount > 55 then
									amount = quantity[j];
									if _dpo then print(resourceAmount..", amount = "..amount); end
								end
							end
				...
				if bCanPlaceResource == true then
					...
					-- fixing a bug: weght reduction and counting must happen only if resource has been placed
					local weightReduction= 1;
					if (resourceInfo.ResourceClassType == "RESOURCECLASS_STRATEGIC") then
						weightReduction = 2;
						enorS = enorS+1
					else
					...
					-- dbg observation running weights
					-- print("resource: ", resourceInfo.Type, " amount: ", amount, " x: ", plot:GetX(), " y: ", plot:GetY());
					if _dpo then print("("..plot:GetX()..", "..plot:GetY()..") "..resourceInfo.Type.." amount: "..amount.." Weight: "..resourceTrackingTable[resourceID].RunningWeight); end
					enor = enor+1
				end
			end
		end
	...
	if _dpo then print("Plots: "..iplots..", Density Passed: "..idensitypass..", Percent: "..tostring(idensitypass/(iplots/100))); end
	if _dpo then print("Exact number of resources: "..enor.." = "..enorS.."("..tostring(enorS/(enor/100)).."%) + "..enorB.."("..tostring(enorB/(enor/100)).."%)"); end
	
	-- a second test before weight reduction bug fix. OBSOLETE
	-- and strategic quantity precent test. ACTUAL
	if 1==0 then
		enor = 0;
		enorS = 0;
		enorB = 0;
		local StrategicQuantity = 0;
		for resourceInfo in GameInfo.Resources("ResourceClassType =\"" .. GameInfo.ResourceClasses["RESOURCECLASS_STRATEGIC"].Type .. "\"") do
			local rt = 0;
			for i, plot in Plots() do
				if plot:GetResourceType() == resourceInfo.ID then rt = rt + 1 end;
			end
			print(rt .. "	" .. resourceInfo.Type);
			enor = enor + rt;
			enorS = enorS+rt
			StrategicQuantity = StrategicQuantity + Map.GetNumResources(resourceInfo.ID);
		end	
		
		-- for resourceInfo in GameInfo.Resources("ResourceClassType = 'RESOURCECLASS_BASIC' OR ResourceClassType = 'RESOURCECLASS_STRATEGIC'") do
		for resourceInfo in GameInfo.Resources("ResourceClassType =\"" .. GameInfo.ResourceClasses["RESOURCECLASS_BASIC"].Type .. "\"") do
			print(Map.GetNumResources(resourceInfo.ID) .. "	" .. resourceInfo.Type);
			enor = enor + Map.GetNumResources(resourceInfo.ID);
			enorB = enorB+Map.GetNumResources(resourceInfo.ID);
		end	
		print("Exact number of resources 2: "..enor.." = "..enorS.."("..tostring(enorS/(enor/100)).."%) + "..enorB.."("..tostring(enorB/(enor/100)).."%)");

		-- this is for test quantity of strategic in all tiles
		print("STRATEGIC PERCENT QUANTITY, PERCENT TILES, QUANTITY PER TILE DATA GATHERING");
		local Strategic1Pt = enorS/100;
		local Strategic1Pq = StrategicQuantity/100;
		for resourceInfo in GameInfo.Resources("ResourceClassType =\"" .. GameInfo.ResourceClasses["RESOURCECLASS_STRATEGIC"].Type .. "\"") do
			local rt = 0;
			for i, plot in Plots() do
				if plot:GetResourceType() == resourceInfo.ID then rt = rt + 1 end;
			...
			print(resourceInfo.Type.."	Quantity "..tostring(Map.GetNumResources(resourceInfo.ID)/Strategic1Pq).."%	Tiles "..tostring(rt/Strategic1Pt).."%	Q per T "..tostring(Map.GetNumResources(resourceInfo.ID)/rt));
		end	
	end
	
	-- Add mandatory titanium, geothermal, petroleum to every start if Strategic Balance option is enabled.
	-- moved to AssignStartingPlots:BalanceCivStarts for balance reasons
	-- if self.resource_setting == 5 then
		-- print("CreateResources starts to do Strategic Balance, resource_setting == 5"); -- dbg
		-- local strategic = {};
		...
		-- for player_num = 1, self.iNumCivs do
			-- if(self.startingPlots[player_num][1] ~= nil and self.startingPlots[player_num][2] ~= nil) then
				...
				-- for i = 1, resourceCount + 1, 1 do
					...
					-- if table.getn(copy) > 0 then	-- bugfix from empty running
...
function AssignStartingPlots:Quantities(resource, amount, min, max)
	...
	-- enhance for 7 RR, each call random
	if (self.resourceQuantity[id][amount] == 0) or self.resource_setting == 7 then
...
function AssignStartingPlots:BalanceAndAssign(args)
	print("AssignStartingPlots:BalanceAndAssign (AssignStartingPlots.Lua)");
	-- dbg print out
	local _dpo = true;
	-- local _dpo = false;
	local args = args or {};
	local iW, iH = Map.GetGridSize();
	local a_Left, a_Right, a_Bottom, a_Top = 3, iW-3, 3, iH-3
	...
	-- Work out the method 6 - Teams West East
	if self.method == 6 then
		...
		for loop, player_ID in ipairs(eastListShuffled) do
			local x = self.startingPlots[loop + self.iNumCivsInWest][1];
			local y = self.startingPlots[loop + self.iNumCivsInWest][2];
			local start_plot = Map.GetPlot(x, y)
			local player = Players[player_ID]
			player:SetStartingPlot(start_plot)
			civ_set[player_ID+1] = true;
			region_status[loop + self.iNumCivsInWest] = true;
		end
	end

	-- Now set fixed SL if they are
	if self.startLocs then
	-- if self.startLocs == 0 then
		if _dpo then print("-Lets do fixed starting locations- Second part"); end
		for loop = 1, self.iNumCivs do
			local player_ID = self.player_ID_list[loop];
			local player = Players[player_ID];
			-- if self.startLocs[("player"..loop)] then
			if self.startLocs[loop] and not civ_set[player_ID+1] then
				-- local start_plot = Map.GetPlotByIndex(self.startLocs[("player"..loop)])
				local start_plot = Map.GetPlotByIndex(self.startLocs[loop])
				civ_set[player_ID+1] = true;
				region_status[loop] = true;
				local x = self.startingPlots[loop][1];
				local y = self.startingPlots[loop][2];
				player:SetStartingPlot(start_plot);
				if _dpo then print("Player: "..loop.." Is Fixed Starting "..x..", "..y); end
			end
		end
	end
...
function AssignStartingPlots:BalanceCivStarts()
	print("AssignStartingPlots:BalanceCivStarts (AssignStartingPlots.Lua)");
	...
	-- 
	-- Anchor Ceti: But thist func does nothing to create an affinity res.
	-- lets fix it.
	-- 1) Check if there more then 1 affinity res in 1,2,3 base ring - then skip creation of starting affinity res.
	-- 2) Make a list of 3rd ring, check each tile for everything, shuffle.
	-- 3) try to place affinity res.
	-- 
	-- The placing of strategic resource by self.resource_setting == 5 Strategic Balance option
	-- should be done only after calculating and placing balance Basic resources.
	-- So self.resource_setting == 5 work is moved here from AssignStartingPlots:CreateResources
	-- 
	-- Handling of variant 8 of Resources Option - Competition 
	...
	-- local basic_energy = {};
	-- local strategic_noaffinity = {};
	-- local strategic_affinity = {};
	...
	for resInfo in GameInfo.Resources() do
		for resourceYieldInfo in GameInfo.Resource_YieldChanges("ResourceType = \"" .. resInfo.Type .. "\"") do
			if(resInfo.ResourceClassType == GameInfo.ResourceClasses["RESOURCECLASS_BASIC"].Type ) then
				...
				elseif (resourceYieldInfo.YieldType == GameInfo.Yields["YIELD_CAPITAL"].Type) then -- ac for future
					table.insert(self.basic_influence, resInfo.ID);
				end
			end
		end
		if(resInfo.ResourceClassType == GameInfo.ResourceClasses["RESOURCECLASS_STRATEGIC"].Type ) then
			if (resInfo.Affinity == false) then
				table.insert(self.strategic_noaffinity, resInfo.ID);
			-- elseif (resInfo.Affinity == true) then
			else
				table.insert(self.strategic_affinity, resInfo.ID);
			end
		end
	end	


	if self.resource_setting == 8 then
		self:CompetitionStart()
		
	else
		self:BasicFood(self.basic_food);
		self:BasicEnergyProduction(self.basic_production, true);
		self:BasicEnergyProduction(self.basic_energy, false);
		--
		self:AffinityThirdRing(self.strategic_affinity);
		-- 
		-- Add mandatory titanium, geothermal, petroleum to every start if Strategic Balance option is enabled.
		if self.resource_setting == 5 then
			print("CreateResources starts to do Strategic Balance, resource_setting == 5"); -- dbg
...
function AssignStartingPlots:BasicFood(food)
	print("AssignStartingPlots:BasicFood (AssignStartingPlots.Lua), table.getn(food)="..tostring(table.getn(food)));
							...
							if(self:InvalidBorderRegion(terrainType, adjacentPlot:GetX() , adjacentPlot:GetY() ) == false) then
								if(terrainType ~= TerrainTypes.TERRAIN_OCEAN  or (terrainType == TerrainTypes.TERRAIN_OCEAN and self:ReachableByWaterCity(adjacentPlot:GetX() , adjacentPlot:GetY()) == true)) then
									if(resourcesToSet > 0) then
										...
										print("---! FOOD RESOURCE: "..basic_food[i].." on "..tostring(adjacentPlot:GetX())..", "..tostring(adjacentPlot:GetY())); -- ac
...
function AssignStartingPlots:BasicEnergyProduction(resource, isProduction)
	print("AssignStartingPlots:BasicEnergyProduction (AssignStartingPlots.Lua), table.getn(resource)="..tostring(table.getn(resource)));
...
function AssignStartingPlots:AffinityThirdRing(affinities)
	print("AssignStartingPlots:AffinityThirdRing (AssignStartingPlots.Lua), table.getn(affinities)="..tostring(table.getn(affinities)));
	-- 1) Check if there more then 1 affinity res in 1,2,3 base ring - then skip creation of starting affinity res.
	-- 2) Make a list of 3rd ring, check each tile for everything, shuffle.
	-- 3) try to place affinity res.

	local StartingHasMoreThen1 = table.fill(0, self.iNumCivs);
	local plot = nil;
	-- dbg print out
	-- local _dpo = true;
	local _dpo = false;

	-- Take current condition of startings
	if _dpo then print("// CALCULATE affinities res tiles //"); end
	for loop = 1, self.iNumCivs, 1 do
		if(self.startingPlots[loop][1] ~= nil and self.startingPlots[loop][2] ~= nil) then			
			local x = self.startingPlots[loop][1];
			local y = self.startingPlots[loop][2];
			plot = Map.GetPlot(x, y);
			...
			for dx = -radius, radius do
				for dy = -radius, radius do
					local otherPlot = Map.PlotXYWithRangeCheck(plot:GetX(), plot:GetY(), dx, dy, radius);
					if(otherPlot) then -- check if has affinity res
						if otherPlot:HasResource() then
							if (GameInfo.Resources[otherPlot:GetResourceType()].Affinity == true) then 
								StartingHasMoreThen1[loop] = StartingHasMoreThen1[loop] + 1;
								if _dpo then print("   Affinity Res at: "..tostring(otherPlot:GetX())..", "..tostring(otherPlot:GetY()));	end
							end
						end
					end
				end
		...
		if _dpo then print("StartPlot: "..tostring(loop)..") "..tostring(self.startingPlots[loop][1])..", "..tostring(self.startingPlots[loop][2])..", AffinityTiles = "..tostring(StartingHasMoreThen1[loop])); end
	end

	-- Make a list of suited tiles
	if _dpo then print("// COLLECT potential affinities res tiles //"); end
	for loop = 1, self.iNumCivs, 1 do
		if StartingHasMoreThen1[loop] < 1 then
			if(self.startingPlots[loop][1] ~= nil and self.startingPlots[loop][2] ~= nil) then
				local x = self.startingPlots[loop][1];
				local y = self.startingPlots[loop][2];
				local loopRingPlots = {};
				...
				for dx = -radius, radius do
					for dy = -radius, radius do
						local otherPlot = Map.PlotXYWithRangeCheck(x, y, dx, dy, radius);				
						if(otherPlot) then -- lets make a ring
							local inRing = Map.PlotXYWithRangeCheck(x, y, dx, dy, radius-1);
							if (inRing == nil) then
								if (otherPlot:GetTerrainType() ~= TerrainTypes.TERRAIN_TRENCH) and (not otherPlot:IsCanyon()) and (not otherPlot:IsMountain()) then
									
									local OtherStartingRange2 = false;		-- check its far then 2 tiles away from any player starting
									for loop = 1, self.iNumCivs, 1 do	
										if(self.startingPlots[loop][1] ~= nil and self.startingPlots[loop][2] ~= nil) then
											local rx = self.startingPlots[loop][1];
											local ry = self.startingPlots[loop][2];			
											local radius = 2;
											for dx = -radius, radius do
												for dy = -radius, radius do
													local inRing2 = Map.PlotXYWithRangeCheck(rx, ry, dx, dy, radius);	
													if (inRing2) then
														if (x == dx) and (y == dy) then
															OtherStartingRange2 = true;
														end
													end
												end
											end
									...
									if (not OtherStartingRange2) then
										if _dpo then print(tostring(loop)..") loopRingPlots add "..tostring(otherPlot:GetX())..", "..tostring(otherPlot:GetY())..", "..tostring(otherPlot:GetPlotIndex())); end
										table.insert(loopRingPlots, otherPlot:GetPlotIndex());
									end
								end
							end
						end
				...
				-- Just place a resource
				if _dpo then print("    // Create affinities res tiles, potential: "..tostring(table.getn(loopRingPlots)).." //"); end
				local shuffledResources = GetShuffledCopyOfTable(affinities);
				shuffledPlotIndexes = GetShuffledCopyOfTable(loopRingPlots);
				for r = 1, table.getn(shuffledResources), 1 do
					for i = 1, table.getn(shuffledPlotIndexes), 1 do
						plot = Map.GetPlotByIndex(shuffledPlotIndexes[i]);
						if (plot) then
							if plot:CanHaveResource(shuffledResources[r]) then
								local quantity = 1;
								local quantityTable = {};
								local amountTotal = 0;
								for resInfo in GameInfo.Resources() do
									for resourceQuantityInfo in GameInfo.Resource_QuantityTypes("ResourceType = \"" .. resInfo.Type .. "\"") do
										local id = resInfo.ID;
										if(shuffledResources[r] == id) then
											local inputSetting = 2;
											if (resourceQuantityInfo.Setting == 1 or resourceQuantityInfo.Setting == 3) then
												inputSetting = resourceQuantityInfo.Setting;
											elseif (resourceQuantityInfo.Setting == 7) then
											...
											if (resourceQuantityInfo.Max ~= nil and resourceQuantityInfo.Min ~= nil) then
												if (inputSetting == self.resource_setting or (self.resource_setting == 4 and inputSetting ~= 7) or (self.resource_setting == 5 and inputSetting ~= 7) or (self.resource_setting == 8 and inputSetting ~= 7)) then
													...
													if _dpo then print("Min/Max: "..min.."/"..max.." "..resInfo.Type.." Quantity: "..quantityNumber); end
													amountTotal = amountTotal + 1;
												end
											end
										end
								...
								plot:SetResourceType(shuffledResources[r], quantity);
								if _dpo then print(tostring(loop)..") STRAT AFF SPAWN AT "..tostring(plot:GetX())..", "..tostring(plot:GetY())..", ID="..shuffledResources[r].." quantity="..quantity); end
								break	
							end					
						end
					end
					break -- for one res type
				end
			end
		end
	end
...
function AssignStartingPlots:CompetitionStart()
	print("AssignStartingPlots:CompetitionStart (AssignStartingPlots.Lua)");
	-- 1) Make lists
	-- 2) Clean res
	-- 3) Place res by order
	
	-- TODO: make sure that tile is not a start loc of other player
	...
	if _dpo then print("// LIST AND CLEAN starting tiles //"); end
	for loop = 1, self.iNumCivs, 1 do
		if(self.startingPlots[loop][1] ~= nil and self.startingPlots[loop][2] ~= nil) then			
			local x = self.startingPlots[loop][1];
			local y = self.startingPlots[loop][2];
			...
			for dx = -radius, radius do
				for dy = -radius, radius do
					local otherPlot = Map.PlotXYWithRangeCheck(plot:GetX(), plot:GetY(), dx, dy, radius);
					if(otherPlot) then -- check if has a res and in previous ring
						local inRing2 = Map.PlotXYWithRangeCheck(x, y, dx, dy, radius-1);
						if (inRing2 == nil) then
							if otherPlot:HasResource() then
								if _dpo then print(tostring(loop)..") Clean a res and add 3rd ring plot "..tostring(otherPlot:GetX())..", "..tostring(otherPlot:GetY()).." ID="..tostring(otherPlot:GetResourceType()).." Q="..tostring(otherPlot:GetNumResource()));	end
								otherPlot:SetResourceType(-1);
							else
								if _dpo then print(tostring(loop)..") Add 3rd ring plot "..tostring(otherPlot:GetX())..", "..tostring(otherPlot:GetY()));	end
							end
							table.insert(ListRing3, otherPlot:GetPlotIndex());
						else
							local inRing1 = Map.PlotXYWithRangeCheck(x, y, dx, dy, radius-2);
							if (inRing1 == nil) then
								if otherPlot:HasResource() then
									if _dpo then print(tostring(loop)..") Clean a res and add 2nd ring plot "..tostring(otherPlot:GetX())..", "..tostring(otherPlot:GetY()).." ID="..tostring(otherPlot:GetResourceType()).." Q="..tostring(otherPlot:GetNumResource()));	end
									otherPlot:SetResourceType(-1);
								else
									if _dpo then print(tostring(loop)..") Add 2nd ring plot "..tostring(otherPlot:GetX())..", "..tostring(otherPlot:GetY()));	end
								end
								table.insert(ListRing2, otherPlot:GetPlotIndex());
							else
								if (not(otherPlot:GetX() == x and otherPlot:GetY() == y)) then
									if otherPlot:HasResource() then
										if _dpo then print(tostring(loop)..") Clean a res and add 1st ring plot "..tostring(otherPlot:GetX())..", "..tostring(otherPlot:GetY()).." ID="..tostring(otherPlot:GetResourceType()).." Q="..tostring(otherPlot:GetNumResource()));	end
										otherPlot:SetResourceType(-1);
									else
										if _dpo then print(tostring(loop)..") Add 1st ring plot "..tostring(otherPlot:GetX())..", "..tostring(otherPlot:GetY()));	end
									end
									table.insert(ListRing1, otherPlot:GetPlotIndex());
								end
							end
						end
					end
			...
			-- There is conditions for spawn a resource. For example: Gold and Floatstone require hill without forest.
			if table.getn(self.strategic_affinity) > 0 then
				local resourcesID = GetShuffledCopyOfTable(self.strategic_affinity);
				local RingOrderedPlots = {};
				local temp_shuffledPlots = GetShuffledCopyOfTable(ListRing3);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				if _dpo then print(tostring(loop)..") Adding strategic_affinity, Shuffled Number of Types:"..tostring(table.getn(resourcesID))..", Number of Ring Ordered Plots:"..tostring(table.getn(RingOrderedPlots))); end
				for r = 1, table.getn(resourcesID), 1 do
					for i = 1, table.getn(RingOrderedPlots), 1 do
			...
			if table.getn(self.basic_culture) > 0 then
				local resourcesID = GetShuffledCopyOfTable(self.basic_culture);
				local RingOrderedPlots = {};
				local temp_shuffledPlots = GetShuffledCopyOfTable(ListRing2);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				temp_shuffledPlots = GetShuffledCopyOfTable(ListRing3);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				if _dpo then print(tostring(loop)..") Adding basic_culture, Shuffled Number of Types:"..tostring(table.getn(resourcesID))..", Number of Ring Ordered Plots:"..tostring(table.getn(RingOrderedPlots))); end
				for r = 1, table.getn(resourcesID), 1 do
					for i = 1, table.getn(RingOrderedPlots), 1 do
						plot = Map.GetPlotByIndex(RingOrderedPlots[i]);
						if (plot) then
							if plot:CanHaveResource(resourcesID[r]) then
								plot:SetResourceType(resourcesID[r], 1);
								if _dpo then print(tostring(loop)..") Plot "..tostring(plot:GetX())..", "..tostring(plot:GetY())..", ID="..resourcesID[r]); end
								break	
							end
						end
					end
			...
			if table.getn(self.strategic_noaffinity) > 0 then
				local resourcesID = GetShuffledCopyOfTable(self.strategic_noaffinity);
				local RingOrderedPlots = {};
				local temp_shuffledPlots = GetShuffledCopyOfTable(ListRing3);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				if _dpo then print(tostring(loop)..") Adding strategic_noaffinity, Shuffled Number of Types:"..tostring(table.getn(resourcesID))..", Number of Ring Ordered Plots:"..tostring(table.getn(RingOrderedPlots))); end				
				for r = 1, table.getn(resourcesID), 1 do
					for i = 1, table.getn(RingOrderedPlots), 1 do
						plot = Map.GetPlotByIndex(RingOrderedPlots[i]);
						if (plot) then
			...
			if table.getn(self.basic_food) > 0 then
				local resourcesID = GetShuffledCopyOfTable(self.basic_food);
				local RingOrderedPlots = {};
				local temp_shuffledPlots = GetShuffledCopyOfTable(ListRing1);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				temp_shuffledPlots = GetShuffledCopyOfTable(ListRing2);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				temp_shuffledPlots = GetShuffledCopyOfTable(ListRing3);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				if _dpo then print(tostring(loop)..") Adding basic_food, Shuffled Number of Types:"..tostring(table.getn(resourcesID))..", Number of Ring Ordered Plots:"..tostring(table.getn(RingOrderedPlots))); end
				for r = 1, table.getn(resourcesID), 1 do
					for i = 1, table.getn(RingOrderedPlots), 1 do
						plot = Map.GetPlotByIndex(RingOrderedPlots[i]);
						if (plot) then
			...
			if table.getn(self.basic_production) > 0 then
				local resourcesID = GetShuffledCopyOfTable(self.basic_production);
				local RingOrderedPlots = {};
				local temp_shuffledPlots = GetShuffledCopyOfTable(ListRing2);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				temp_shuffledPlots = GetShuffledCopyOfTable(ListRing3);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				if _dpo then print(tostring(loop)..") Adding basic_production, Shuffled Number of Types:"..tostring(table.getn(resourcesID))..", Number of Ring Ordered Plots:"..tostring(table.getn(RingOrderedPlots))); end
				for r = 1, table.getn(resourcesID), 1 do
					for i = 1, table.getn(RingOrderedPlots), 1 do
						plot = Map.GetPlotByIndex(RingOrderedPlots[i]);
						if (plot) then
			...
			if table.getn(self.basic_energy) > 0 then
				local resourcesID = GetShuffledCopyOfTable(self.basic_energy);
				local RingOrderedPlots = {};
				local temp_shuffledPlots = GetShuffledCopyOfTable(ListRing2);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				temp_shuffledPlots = GetShuffledCopyOfTable(ListRing3);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				if _dpo then print(tostring(loop)..") Adding basic_energy, Shuffled Number of Types:"..tostring(table.getn(resourcesID))..", Number of Ring Ordered Plots:"..tostring(table.getn(RingOrderedPlots))); end
				for r = 1, table.getn(resourcesID), 1 do
					for i = 1, table.getn(RingOrderedPlots), 1 do
						plot = Map.GetPlotByIndex(RingOrderedPlots[i]);
						if (plot) then
			...
			if table.getn(self.basic_science) > 0 then
				local resourcesID = GetShuffledCopyOfTable(self.basic_science);
				local RingOrderedPlots = {};
				local temp_shuffledPlots = GetShuffledCopyOfTable(ListRing2);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				temp_shuffledPlots = GetShuffledCopyOfTable(ListRing3);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				if _dpo then print(tostring(loop)..") Adding basic_science, Shuffled Number of Types:"..tostring(table.getn(resourcesID))..", Number of Ring Ordered Plots:"..tostring(table.getn(RingOrderedPlots))); end
				for r = 1, table.getn(resourcesID), 1 do
					for i = 1, table.getn(RingOrderedPlots), 1 do
						plot = Map.GetPlotByIndex(RingOrderedPlots[i]);
						if (plot) then
			...
			if table.getn(self.basic_influence) > 0 then
				local resourcesID = GetShuffledCopyOfTable(self.basic_influence);
				local RingOrderedPlots = {};
				local temp_shuffledPlots = GetShuffledCopyOfTable(ListRing2);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				temp_shuffledPlots = GetShuffledCopyOfTable(ListRing3);
				for i = 1, table.getn(temp_shuffledPlots), 1 do table.insert(RingOrderedPlots, temp_shuffledPlots[i]); end
				if _dpo then print(tostring(loop)..") Adding basic_influence, Shuffled Number of Types:"..tostring(table.getn(resourcesID))..", Number of Ring Ordered Plots:"..tostring(table.getn(RingOrderedPlots))); end
				for r = 1, table.getn(resourcesID), 1 do
					for i = 1, table.getn(RingOrderedPlots), 1 do
						plot = Map.GetPlotByIndex(RingOrderedPlots[i]);
						if (plot) then
	...
	if _dpo then print("// COMPETITION starting res done //"); end
end
------------------------------------------------------------------------------
function AssignStartingPlots:SetResources(resources, startPlot, targetRange, plotIndex, count)
	print("AssignStartingPlots:SetResources (AssignStartingPlots.Lua), table.getn(resources)="..tostring(table.getn(resources)).." at "..tostring(startPlot:GetX())..", "..tostring(startPlot:GetY())..", targetRange="..tostring(targetRange)..", count="..tostring(count)..", self.resourcesSet="..tostring(self.resourcesSet));
	...
	if (plot == nil) then
		if _dpo then print("---return by plot"); end
		return;
	end
	...
	if(count > 3 or self.resourcesSet <= 0) then
		if _dpo then print("---return by count("..tostring(count)..") > 3 or self.resourcesSet("..tostring(self.resourcesSet)..") <= 0"); end
		return;
	end
	...
	for loop = 0, numDirections - 1, 1 do
		local valid = true;
		-- local adjacentPlot = Map.PlotDirection(x, y, direction); -- HERE IS A FATAL ERROR. 
		-- WHEN THIS FUNCTION TRY TO CALL A TILE OUT OF BORDERS WHEN PLAYER STARTING AT THE EDGE OF MAP -- THEN ALL THE SCRIPT FILE STOPS WITHOUT ANY SIGNS OF ERROR.
		-- when trying to send wrong index through self:SetResources
		-- need to check edge tile coordinates and fix it before proceed;
		if y == 0 then y = 1 end;
		if y == iH-1 then y = iH-2 end;
		-- squared map, not wrapped
		if Map:IsWrapX() == false then
			if x == 0 then x = 1 end;
			if x == iW-1 then x = iW-2 end;
		end
		local adjacentPlot = Map.PlotDirection(x, y, direction);
		if _dpo then print("count("..tostring(count)..") loop("..tostring(loop)..") to random direction and get adjacentPlot: "..tostring(adjacentPlot:GetX())..", "..tostring(adjacentPlot:GetY())); end

		if (self.resourcesSet <= 0) then
			if _dpo then print("---return by self.resourcesSet("..tostring(self.resourcesSet)..") <= 0 in loop"); end
			return
		end
		...
		for i, index in ipairs(plotIndex) do
			if (adjacentPlot == nil or (index ~= nil and index == adjacentPlot:GetPlotIndex())) then
				valid = false;
				if _dpo then print("adjacentPlot been processed"); end
			end
		end
							...
							if(terrainType ~= TerrainTypes.TERRAIN_OCEAN  or (terrainType == TerrainTypes.TERRAIN_OCEAN and self:ReachableByWaterCity(adjacentPlot:GetX() , adjacentPlot:GetY() ) == true) or (terrainType == TerrainTypes.TERRAIN_OCEAN and self.reach == true)) then
								if(self.resourcesSet > 0) then
									...
									if(adjacentPlot:CanHaveResource(shuffledResources[i])) then
										...
										if _dpo then print("---! SET RESOURCE: "..adjacentPlot:GetX()..", "..adjacentPlot:GetY()..", ID="..shuffledResources[i].." quantity="..quantity); end
										self.resourcesSet = self.resourcesSet - 1;
									end
								else
									if _dpo then print("self.resourcesSet <= 0"); end
								end
							else
								if _dpo then print("Not Land or Not Reachable by WaterCity"); end
							end
						else
							if _dpo then print("InvalidBorderRegion of adjacentPlot"); end
						end
					else
						if _dpo then print("adjacentPlot cant have resID="..shuffledResources[i].." or Impossible or Trench"); end				
					end
				end
			else
				if _dpo then print("count("..tostring(count)..") < targetRange("..tostring(targetRange)..")"); end				
			end
		else
			if _dpo then print("valid = false"); end
		end

		if _dpo then print("Self start"); end	
	...
	-- what is this? direction changer should be inside loop of directions!!!
	-- direction = direction + 1;
	-- if (direction == numDirections - 1) then
		-- direction = 0;
	-- end
...
function AssignStartingPlots:PlaceResourcesAndCityStates()
	...
	local fertilityMap = CalculateFertilityMap();
