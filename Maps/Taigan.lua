...
function GeneratePlotTypes()
	...
	if userInputLandmass == 5 then -- Random
		userInputLandmass = 1 + Map.Rand(6, "Taigan Random Landmass Type - Lua");
...
function FeatureGenerator:AddFeatures(allow_mountains_on_coast)
	print("FeatureGenerator:AddFeatures(allow_mountains_on_coast) - Taigan.lua");
	...
	if self.miasmaSpawnWay == 1 then -- no miasma
	
	elseif self.miasmaSpawnWay == 2 then -- miasma before wildness
		self:AddMiasma();
	end	
	...
	-- self:AddMiasma();
	if self.miasmaSpawnWay == 3 then -- miasma after wildness, Default
		self:AddMiasma();
	
	elseif self.miasmaSpawnWay == 4 then -- miasma all over
		for y = 0, self.iGridH - 1 do
			for x = 0, self.iGridW - 1 do
				local mplot = Map.GetPlot(x, y)
				if (mplot:IsWater() or mplot:IsMountain()) then
				else
					mplot:SetMiasma(true);
				end
			end
		end
	end
...
function StartPlotSystem()
	...
	local res = Map.GetCustomOption(3) -- this map has Resource option on number 3
	if res == 6 then
	...
	print("Choosing start locations for civilizations - Taigan Lua.");
	local args = {};
	if userInputLandmass == 3 then
