...
---- MGH some minor changes for generating more quantity of ice
-- ===========================================================================
-------------------------------------------------------------------------------
--	FILE:	 Ice_Age.lua
--	AUTHOR:  Bob Thomas
...
function GetMapScriptInfo()
	...
	return {
		...
		Type = "TXT_KEY_MAP_ICE_AGE_TYPE",--MGH
...
function FeatureGenerator:AddIceAtPlot(plot, iX, iY, lat)
	--[[if iY == 0 or iY == self.iGridH - 1)--MGH: Poles not accesibles only
		--plot:SetTerrainType(GameInfo.Terrains["TERRAIN_OCEAN"].ID);
		plot:SetTerrainType(TerrainTypes.TERRAIN_OCEAN);
	end]]--
	if(plot:CanHaveFeature(self.featureIce)) then
	
		if iY == 0 or iY == self.iGridH - 1 then --MGH: 1 border area is ice
			plot:SetFeatureType(self.featureIce, -1)
		elseif lat > 0.47 then
			local rand = Map.Rand(100, "Add Ice Lua") / 100;
			if rand < (0.32 + (5 * (lat - 0.48))) then --MGH: old: 8 * (lat - 0.50) and removed 4 * (lat - 0.46)
				plot:SetFeatureType(self.featureIce, -1)
			end
		...
		elseif lat > 0.39 then
			...
			if rand < 0.26 then --MGH: old:0.06
				plot:SetFeatureType(self.featureIce, -1)
			end
		elseif lat > 0.32 then
			local rand = Map.Rand(100, "Add Encroaching Ice - Sirian's Ice Age - Lua") / 100;
			if rand < 0.20 then --MGH: old:0.04
				plot:SetFeatureType(self.featureIce, -1)
			end
		elseif lat > 0.27 then
			local rand = Map.Rand(100, "Add Encroaching Ice - Sirian's Ice Age - Lua") / 100;
			if rand < 0.14 then --MGH: old: 0.02
				plot:SetFeatureType(self.featureIce, -1)
			end
		end
	end
...
function AddFeatures()
	...
	local args = {miasmaSpawnWay = miasma,}
