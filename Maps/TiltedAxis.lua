...
---- MGH bugs with some features appearing on sea solved
-- ===========================================================================
------------------------------------------------------------------------------
--	FILE:	 TiltedAxis.lua
--	AUTHOR:  Bob Thomas
		...
		if(plot:CanHaveFeature(self.featureJungle)) then
			...
			if repurpose_height > self.iTop then
				...
				if plotType ~= PlotTypes.PLOT_HILLS then
					if plot:CanHaveFeature(self.featureMarsh) then --MGH bug CanHaveFeature fixed
						plot:SetFeatureType(self.featureMarsh, -1);
					end --MGH end bug CanHaveFeature fixed
				end
			elseif repurpose_height < self.iBottom then
				if plot:CanHaveFeature(self.featureForest) then --MGH bug CanHaveFeature fixed
